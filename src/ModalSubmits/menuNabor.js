...
module.exports = {
    ...
    async execute(client, interaction) {
        ...
        switch (type) {
            case 'ControlModNab1':
                label = ['Имя и возраст', 'Есть ли опыт работы в подобной сфере?', 'Были ли у тебя муты/варны/баны?', 'В какое время тебе удобно работать?', 'Расскажи немного о себе', 'заявки на Модератора']
                channelId = ChannelID.Control
                content = `<@&${RoleID.ControlMentor}>`
                break;
            case 'ControlModNab2':
                label = ['Имя и возраст', 'Есть ли опыт работы в подобной сфере?', 'Были ли у тебя муты/варны/баны?', 'В какое время тебе удобно работать?', 'Почему именно ты?', 'заявки на Модератора']
                channelId = ChannelID.Assist
                content = `<@&${RoleID.ControlMentor}>`
                break;
            case 'EventModNab':
                label = ['Имя, возраст, часовой пояс', 'Был ли опыт в проведении ивентов? Где?', 'Какие ивенты готов(а) / хочешь проводить?', 'Почему именно ветка ивентёров?', 'Стоите ли вы в стаффе другого сервера?', 'заявки на Ивентера']
                channelId = ChannelID.Event
                content = `<@&${RoleID.EventMentor}>`
                break;
            case 'MafiaModNab1':
                label = ['Был ли опыт в проведении мафии до заявки?', 'Сколько игр на нашем сервере?', 'Сколько можно взять +30 за круг?', 'Что будет после 3-ех фолов у игрока?', 'Что вы сделаете за клятву во время игры?', 'заявки на Ведущего мафии ']
                channelId = ChannelID.Mafia
                content = `<@&${RoleID.MafiaMentor}>`
                break;
            case 'MafiaModNab2':
                label = ['Имя и возраст', 'Напиши немного о себе', 'Сколько времени можете уделять должности?', 'Сколько игр в бункер вы наиграли?', 'Почему именно ты?', 'заявки на Ведущего бункера']
                channelId = ChannelID.Bunker
                content = `<@&${RoleID.BunkerMentor}>`
                break;
            case 'CloseModNab':
                label = ['Имя и возраст', 'Какую игру хочешь проводить?', 'Был ли опыт в проведении клозов?', 'Когда тебе удобно проводить клозы?', 'Расскажи немного о себе', 'заявки на Клозера']
                channelId = ChannelID.Close
                content = `<@&${RoleID.CloseMentor}>`
                break;
            case 'CreativeModNab':
                label = ['Имя и возраст', 'Выбери направление', 'Есть ли опыт работы в данных областях?', 'Сколько времени можешь уделять должности?', 'Почему именно ты?', 'заявки на Креатива']
                channelId = ChannelID.Creative
                content = `<@&${RoleID.CreativeMentor}>`
                break;
            case 'ContentModNab':
                label = ['Твое имя, возраст, часовой пояс', 'Сколько времени можешь уделять должности?', 'Расскажи о себе, своем прошлом опыте', 'Чем бы ты хотел(а) заниматься?', 'Почему именно ты?', 'заявки на Контентера']
